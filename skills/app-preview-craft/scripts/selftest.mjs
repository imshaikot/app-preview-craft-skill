...
try {
  ...
  await check('studio: previews load, a drag becomes an override, a transcript line and an undo step', async () => {
    ...
    try {
      ...
      await page.waitForFunction((want) => Math.abs(document.querySelector('.frame[data-index="3"] iframe').contentWindow.stage.three.devices[0].group.position.x - want) < 1, { timeout: ci ? 600_000 : 20_000 }, during)
      const stored = () => page.evaluate(() => JSON.parse(localStorage.getItem(Object.keys(localStorage)[0])).overridesByCat['app-store'])
      await page.waitForFunction(() => Object.keys(localStorage).length > 0, { timeout: ci ? 600_000 : 5000 })
      assert((await stored())?.device?.x > 0.5, `no device.x override after the drag: ${JSON.stringify(await stored())}`)
      await page.waitForFunction(() => fetch('/api/transcript').then((r) => r.json()).then((t) => t.events.some((e) => e.type === 'preview.edit')), { timeout: ci ? 600_000 : 10_000 })
      ...
      await page.waitForFunction(() => !JSON.parse(localStorage.getItem(Object.keys(localStorage)[0])).overridesByCat['app-store']?.device, { timeout: ci ? 600_000 : 5000 })
      await settled(5)
